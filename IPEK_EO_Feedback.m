...
v_min = 0; v_max = 4; v_step = 0.01; %0.01; 
...
P_ratio = 0.5; % How much of P_max we want our high-state voltage to be
...
% Finding the power and voltage of our "high-state"
V_working = measured_V(possible_indices(1));
P_working = agilent_results(possible_indices(1));
...
output_str = sprintf('Calculated values: a = %.4f V^-1, b = %.4f V/mW, V_offset = %.4f V, P_working = %.4f mW, V_working = %.4f V', ...
                     a, b, V_offset, P_working, V_working);
...
yyaxis left
plot(iterations, tuning_voltage_arr, '-o', 'Color', 'r', 'DisplayName', 'Tuning Voltage');
ylabel('Voltage (V)');
